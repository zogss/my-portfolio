...
| A new project entry                  | Append to [public/projects.json](public/projects.json), add `*_description`, `*_long_description_NN` and the three `*_case_{problem,approach,outcome}` keys to both locale JSONs, drop the card image into [public/images/projects/](public/images/projects/) (plus an optional `carousel` array pointing at `public/images/projects/<slug>/`), then add the slug to `ProjectSlugType`/`ProjectNameType` and to the colour maps in all three [src/components/variants/](src/components/variants/) components |
